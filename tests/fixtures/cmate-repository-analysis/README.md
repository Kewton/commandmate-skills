...
`.commandmate/verify.yaml` の `repository-analysis-fixtures` ゲートと
`.github/workflows/validate.yml` から実行される。
0.2.0 より前はどちらにも登録が無く、**CI で一度も回っていなかった**（Issue #70）。
...
evidence が fixture 内に実在すること、行番号が file の行数内にあること、
除外すべき path を根拠にしていないこと、走査範囲の申告が数字と矛盾しないこと、
そして **fixture の secret 値が result のどこにも現れないこと**。

0.2.0 で schema が advisory になったので、schema 違反は
**助言として印字するだけで合否を変えない**。落とすのは上の規律だけである。
唯一の例外が2つの閉じた形 —— `evidence` と `sensitive_locations` —— で、
ここに未知 field を足すことは規律違反として落とす。
`snippet` や `value_preview` を足せてしまえば、
「位置だけを渡す」という設計はその瞬間に無効になるからである。
...
`samples/` に **落とされるべき result** を6件置いてある。
何でも受理する採点器は、何も採点していないのと同じである。
`--selftest` が緑であることは、採点器がそれらを実際に落とせることまで含む。
schema を緩めた 0.2.0 で確かめるべきなのは、まさにこの6件が
**素通りするようになっていないこと** である。
...
- `nextjs-app.quoted-evidence.json` — evidence に source 行を、`sensitive_locations` に値を足している

最後の1件は 0.2.0 で足した。closed schema なら「未知 field」で落ちていたもので、
schema を緩めた後も落ちることを、採点器自身の規律として固定するためにある。

対になる `nextjs-app.extra-fields.json` は **受理されなければならない**。
schema が知らない field（`analysis_duration_ms` など）を持つが、
どれも evidence 規律には触れていない。格下げが実際に効いていることは、
この1件が緑であることで確かめる。
...
実施済みなのは `--selftest`（採点器が参考 result 5件を受理し、
落とすべき result 6件を落とすこと）だけである。
