...
| `send` | `<worktree-id> <message>` | exit 0 で送信成功（task id は返らない） | generic worker prompt を dispatch |
| `wait` | `<worktree-id> --timeout <sec>` | **exit code**: 0 completed / 10 prompt / 124 timeout / その他 failed | worker の終端を待つ（block） |
| `capture` | `<worktree-id> --json` | `{ "content", "promptData": { "question" }, … }` | prompt/出力を human 提示用に取得 |
...
全 command が exit 0 なら pass とする（第2.1節）。
