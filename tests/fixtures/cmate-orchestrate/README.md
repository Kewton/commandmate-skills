...
**この version（0.3.0）の時点で、実機評価は未実施である。**
実施済みなのは `run_tests.mjs`（9 plan case + 7 dispatch case + 12 merge case が緑）だけである。
dispatch の実機確認（2 Issue / 2 並列の dispatch→wait→verification）と、PR 作成→CI 確認→merge の
実機確認（2 Issue）は live 環境で別途行う。
