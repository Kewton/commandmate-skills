...
`uat_scenario` が UAT の合否（`pass_on` で attempt ごとに変える）、fix worker の完了・検証、fix worktree
作成の可否、再merge の conflict を注入する。`fake-cli.mjs` は各呼び出しを `CMATE_FAKE_LOG` に記録する
ので、preview で worktree 作成・fix dispatch・再merge が呼ばれていないこと、修正ループが上限で停止
（回数無制限でない）していること、attempt 履歴が上書きでなく append されていることまで検証できる。
