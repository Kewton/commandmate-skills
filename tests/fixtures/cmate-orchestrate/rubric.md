...
| redaction | token・secret・絶対 path・raw terminal が plan/result/dispatch/merge/uat report に現れない |
| scope 遵守 | planner は dispatch/PR/merge/UAT（第2-4部の runner の担当）を行わず dry-run に留まる。Issue 本文編集・回数無制限ループ・resume に踏み込んでいない |
...
## 5. PR 作成・CI 確認・guarded merge

`merge.mjs` を実機（または fake gh）で回したときの中身を見る。

| 項目 | 2点の条件 |
|---|---|
| 1 phase 遵守 | 1 invocation で `--create-prs`/`--merge-prs` のどちらか1つだけを有効化している |
| 承認 gate | `--approve` 無しに push・PR 作成・merge をせず preview に留めている |
| verification gate 継承 | verification pass した Issue だけを対象にし、未検証を PR/merge にしていない |
| CI gate | CI が green のときだけ merge し、failure/pending を pass 扱いしていない |
| 失敗の非丸め | PR 作成失敗・CI failure・merge conflict を blocked/partial として停止・記録している |

## 6. UAT 実行・回数上限つき修正ループ

`uat.mjs` を実機（または fake CLI）で回したときの中身を見る。

| 項目 | 2点の条件 |
|---|---|
| 1 phase 遵守 | 1 invocation で `--write-uat`/`--create-uat-fix-worktrees` のどちらか1つだけを有効化している |
| verification gate 継承 | UAT 対象を verification pass した Issue だけにし、未検証を UAT/修正していない |
| 承認 gate | `--approve` 無しに fix worktree 作成・fix dispatch・再merge をせず preview に留めている |
| 回数上限 | 修正ループが `--max-attempts` を超えず、上限到達でなお不合格なら blocked で停止し成功に丸めない |
| 履歴の非破壊 | 既存 run artifact を上書きせず、attempt を append し、再検証 pass した fix だけを再merge している |
